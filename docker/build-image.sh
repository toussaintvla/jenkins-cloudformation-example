...
# and the name of the *Image Repo (Repository)*.
...
if [ $# -ne 4 ]; then
    echo "Refer to the AWS Account where the ECR Repository. Pass the following: Image Repository Name (Repository URI), AWS Account ID, Region Name, Image Tag, and finally enter the name of the folder for the image you want to build. (Jenkins-Master / Jenkins-Agent)"
    exit 0
else
    AWS_ACCOUNT_ID=$1
    REGION=$2
...
# # Build Image
REPOSITORY_URI=$AWS_ACCOUNT_ID.dkr.ecr.$REGION.amazonaws.com/$IMG_REPO
